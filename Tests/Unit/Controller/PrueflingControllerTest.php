...
class PrueflingControllerTest extends \TYPO3\CMS\Core\Tests\UnitTestCase {
    ...
    public function listActionFetchesAllPrueflingsFromRepositoryAndAssignsThemToView() {
        ...
        $allPrueflings = $this->getMock(self::OBJECTSTORAGE, array(), array(), '', FALSE);
        $allFeUserGroups = $this->getMock(self::OBJECTSTORAGE, array(), array(), '', FALSE);

        $fachRepository = $this->getMock('ReRe\\Rere\\Domain\\Repository\\FachRepository', array(self::FINDBYUID), array(), '', FALSE);
        ...
        $modulRepository = $this->getMock('ReRe\\Rere\\Domain\\Repository\\ModulRepository', array(self::FINDBYUID), array(), '', FALSE);
        ...
        $prueflingRepository = $this->getMock(self::PRUEFLINGSREPOSITORY, array(self::FINDALL), array(), '', FALSE);
        $prueflingRepository->expects($this->once())->method(self::FINDALL)->will($this->returnValue($allPrueflings));
        $this->inject($this->subject, self::PRUEFLINGSREPO, $prueflingRepository);

        $frontendUserGroupRepository = $this->getMock(self::FRONTENDUSERGROUPREPOSITORY, array(self::FINDALL), array(), '', FALSE);
        $frontendUserGroupRepository->expects($this->once())->method(self::FINDALL)->will($this->returnValue($allFeUserGroups));
    ...
    public function newActionAssignsTheGivenPrueflingToView() {
        ...
        $request->expects($this->any())->method(self::GETARGUMENT)->will($this->returnValue($name));
        $request->expects($this->any())->method(self::GETARGUMENT)->will($this->returnValue($vorname));
        $request->expects($this->any())->method(self::GETARGUMENT)->will($this->returnValue($email));
        $request->expects($this->any())->method(self::GETARGUMENT)->will($this->returnValue($matrikelnr));
        $this->inject($this->subject, 'request', $request);
