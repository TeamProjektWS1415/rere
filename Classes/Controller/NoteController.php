...
class NoteController extends \TYPO3\CMS\Extbase\Mvc\Controller\ActionController {
	...
	foreach ($notes as $note) {
		...
		array_push($publisharray, array('prueflingvorname' => $pruefling->getVorname(), 'matrikelnr' => $pruefling->getMatrikelnr(), 'prueflingnachname' => $pruefling->getNachname(), 'prueflingname' => $pruefling->getPrueflingname(), 'uid' => $note->getUid(), 'wert' => $note->getWert(), 'kommentar' => $note->getKommentar()));
		$angemeldete++;
	    }
	}
